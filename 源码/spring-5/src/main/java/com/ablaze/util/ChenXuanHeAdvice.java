...
public class ChenXuanHeAdvice {
    ...
    public Object aroundPrintLog(ProceedingJoinPoint pjp) {
        ...
        try {
            ...
            System.out.println("chenxuanhe环绕通知记录日志前置");
            result = pjp.proceed(args); // 明确调用业务层方法（切入点方法）
            System.out.println("chenxuanhe环绕通知记录日志后置");
            return result;
        } catch (Throwable e) { // proceed方法抛出了Throwable，这里用Exception拦不住
            System.out.println("chenxuanhe环绕通知记录日志异常");
            e.printStackTrace();
            throw new RuntimeException(e);
        } finally {
            System.out.println("chenxuanhe环绕通知记录日最终");
        } }
}
